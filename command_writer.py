...
class CommandWriter:
    ...
    def get_path_list(self, words_info):
        ...
        # add some initial swipes to 'warm up' sendinput
        path_list = [
            [(-1,0), (-1,1), (-1,2), (-1,3)], 
            [(-1,0), (-1,1), (-1,2), (-1,3)],
            [(-1,0), (-1,1), (-1,2), (-1,3)],
    ...
    def save_commands(self, path_list):
        """writes all sendevent commands to file"""
        commands = ''
        for path in path_list: # each word is stored on 3 lines: length, xvals, yvals
            commands = CommandWriter.start_swipe(commands, CHAR_MID_X0+GAP_X*path[0][1], CHAR_MID_Y0+GAP_Y*path[0][0])
            for x, y in path[1:]:
        ...
        with open(MAIN_DIR / 'ruzzle_swipe.sh', 'w') as file:
            file.write(commands)
